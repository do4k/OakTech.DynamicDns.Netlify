...
public class DomainManagementService : BackgroundService
{
    ...
    {
        _logger.LogInformation("Starting Domain Management Service with Access Token {AccessToken}", _netlifyOptions.AccessToken);
        while (stoppingToken.IsCancellationRequested is false)
        {
            ...
            try
            {
                var dnsZones = await _netlifyApi.GetDnsZonesAsync("oaktech", $"Bearer {_netlifyOptions.AccessToken}");
                ...
                foreach (var record in managedZone.Records.Where(x => x.Managed is false))
                {
                    ...
                    await _netlifyApi.DeleteDnsRecordAsync(record.DnsZoneId, record.Id, $"Bearer {_netlifyOptions.AccessToken}");
                    ...
                    var result = await _netlifyApi.CreateDnsRecordsAsync(record.DnsZoneId, newEntry, $"Bearer {_netlifyOptions.AccessToken}");
